...
use crate::object::{BuiltinFunction, Object, ObjectKey};
...
fn eval_statement(stmt: Statement, env: &mut Environment) -> Object {
    match stmt {
        ...
        Statement::While(while_stmt) => {
            eval_while_statement(while_stmt.condition, while_stmt.body, env)
        }
...
fn eval_while_statement(
    ...
    loop {
        let condition_val = eval_expression(condition.clone(), env);
        if let Object::Error(_) = condition_val {
        ...
        result = eval_block_statement(body.clone(), env);
...
fn eval_for_statement(for_stmt: crate::ast::ForStatement, env: &mut Environment) -> Object {
    let mut result = Object::Null;
    ...
    if let Some(init_stmt) = init {
        eval_statement(*init_stmt, env);
    }

    loop {
        if let Some(cond) = &condition {
            let condition_val = eval_expression(cond.clone(), env);
            if let Object::Error(_) = condition_val {
                return condition_val;
            }
            if !is_truthy(condition_val) {
        ...
        if let Some(update_stmt) = &update {
            eval_statement((**update_stmt).clone(), env);
        }
    }
...
fn eval_expression(expr: Expression, env: &mut Environment) -> Object {
    match expr {
        ...
        Expression::If(if_expr) => eval_if_expression(
            if_expr.condition,
            if_expr.consequence,
            if_expr.alternative,
            env,
        ...
        Expression::Assign(assign) => {
            ...
            env.set(assign.name.value, value);
            Object::Null
        }
...
mod tests {
    ...
    #[test]
    fn test_eval_while_false() {
        let input = "while (false) { 100; }";
        let evaluated = test_eval(input);
        assert_eq!(evaluated, Object::Null);
    }
    ...
    fn test_eval_for_statement() {
        let tests = vec![
            (
                "let sum = 0; for (let i = 0; i < 3; i = i + 1) { sum = sum + i; } sum;",
                3.0,
            ),
            (
                "let sum = 0; for (let i = 1; i <= 5; i = i + 1) { sum = sum + i; } sum;",
                15.0,
            ),
            ("let i = 0; for (; i < 4; i = i + 1) { } i;", 4.0),
        ];
    ...
    fn test_while_return() {
        let input = "let i = 0; while (i < 5) { i = i + 1; if (i == 3) { return i; } }";
        let evaluated = test_eval(input);
        match evaluated {
            Object::Integer(val) => assert_eq!(val, 3.0),
            _ => panic!("Expected Integer(3), got {:?}", evaluated),
        }
    }

    #[test]
    fn test_for_return() {
        let input = "let sum = 0; for (let i = 1; i <= 10; i = i + 1) { sum = sum + i; if (sum > 15) { return sum; } }";
        let evaluated = test_eval(input);
        match evaluated {
            Object::Integer(val) => assert_eq!(val, 21.0),
            _ => panic!("Expected Integer(21), got {:?}", evaluated),
        }
    }
}

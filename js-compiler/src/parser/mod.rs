use crate::ast::{
    ArrayLiteral, AssignmentExpression, BlockStatement, BooleanLiteral, CallExpression, Expression,
    ExpressionStatement, ForStatement, FunctionLiteral, HashLiteral, Identifier, IfExpression,
    InfixExpression, IntegerLiteral, LetStatement, MemberExpression, PrefixExpression, Program,
...
impl Parser {
    ...
    fn get_infix_parse_fn(&self, token: &Token) -> Option<InfixParseFn> {
        match token {
            ...
            | Token::Gt
            | Token::LtEq
            | Token::GtEq => Some(Self::parse_infix_expression),
    ...
    fn parse_hash_literal(&mut self) -> Option<Expression> {
        ...
        Some(Expression::Hash(Box::new(HashLiteral { token, pairs })))
    }
...
mod tests {
    ...
    fn test_identifier_expression() {
        ...
        match &program.statements[0] {
            ...
            _ => panic!(
                "stmt not ExpressionStatement. got={:?}",
    ...
    fn test_infix_expressions() {
        ...
        let infix_tests_identifiers = vec![
            InfixTest {
                input: "a + b;".to_string(),
                left_value: "a".to_string(),
                operator: "+".to_string(),
                right_value: "b".to_string(),
            },
            InfixTest {
                input: "a - b;".to_string(),
                left_value: "a".to_string(),
            ...
            match &program.statements[0] {
                Statement::Expression(stmt) => match &stmt.expression {
                    Expression::Infix(exp) => {
                        ...
                        match &exp.right {
                            Expression::Identifier(ident) => {
                                assert_eq!(ident.value, tt.right_value)
                            }
            ...
            Statement::Expression(stmt) => match &stmt.expression {
                Expression::FunctionLiteral(func) => {
                    ...
                    match &func.body.statements[0] {
                        Statement::Expression(body_stmt) => match &body_stmt.expression {
                            Expression::Infix(infix) => {
                                assert_eq!(infix.operator, "+");
                            }
        ...
        match &program.statements[0] {
            Statement::Expression(stmt) => match &stmt.expression {
                Expression::Hash(hash) => {
                    ...
                    let expected = vec![("one", 1.0), ("two", 2.0), ("three", 3.0)];
